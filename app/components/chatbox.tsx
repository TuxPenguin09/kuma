    ...
    return (
        <div className="flex-1 flex flex-col h-full">
            ...
            <div className={`${sideBarCollapsed ? "w-[calc(100%-10px)]" : "w-[calc(100%-50px)]"} dark:bg-[#3e3e3e] p-[5px] rounded-tl-[10px] rounded-tr-[10px] ${sideBarCollapsed ? "ml-[10px]" : "ml-[50px]"} flex items-end gap-2`}>
                ...
                />
                <div className={`px-4 py-2 bg-blue-600 text-white rounded-md select-none ${!chatSendEnabled ? "opacity-50 cursor-not-allowed pointer-events-none" : "cursor-pointer"}`}>Send</div>
            </div>
        </div>
    );
}